...
df3_sep <- df3_top |>
  ...
  pblapply(function(x){
    ...
    xred$Prediction_nls_woods <- predict(nls_woods)
    xred$Residuals_nls_woods <- residuals(nls_woods)
    ...
    xred$Prediction_nlme_woods <- predict(nlme_woods)
    xred$Residuals_nlme_woods <- residuals(nlme_woods)
    ...
    nls_wilmink <- nls.multstart::nls_multstart(logSCC ~ f_wilmink(DIM, a, b, k, d),
                                              ...
                                              start_upper = c(a=9, b=1.5, k=0, d=5),
                                              iter = 500,
                                              supp_errors = "Y")

    xred$Prediction_nls_wilmink <- predict(nls_wilmink)
    xred$Residuals_nls_wilmink <- residuals(nls_wilmink)
    ...
    xred$Prediction_nlme_wilmink <- predict(nlme_wilmink)
    xred$Residuals_nlme_wilmink <- residuals(nlme_wilmink)

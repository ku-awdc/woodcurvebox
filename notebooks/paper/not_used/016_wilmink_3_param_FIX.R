...
df3_sep <- df3_top |>
  ...
  pblapply(function(x){
    ...
    nls_wilmink <- nls.multstart::nls_multstart(logSCC ~ f_wilmink(DIM, a, b, k),
                                              ...
                                              start_lower = c(a=-100, b=-100, c=-100),
                                              start_upper = c(a=100, b=100, c=100),
                                              iter = 500,
                                              supp_errors = "Y")

    xred$Prediction_nls <- predict(nls_wilmink)
    xred$Residuals_nls <- residuals(nls_wilmink)
    ...
    xred$Prediction_nlme <- predict(nlme_wilmink)
    xred$Residuals_nlme <- residuals(nlme_wilmink)
